...
export const createElementSetTools = () => [
  ...
  defineTool('listElementSets', '列出所有元素集（网页元素选择器的集合）。搭建工作流需要网页元素时，先调用本工具查看现有元素集，优先匹配复用，不要每次都新建。', {
    type: 'object',
    properties: {
  ...
  defineTool('getElementSet', '获取单个元素集的完整详情（含元素与选择器）。取出的元素对象可直接作为浏览器节点 config 的 selector 字段值（节点存元素副本，与元素集无 ID 关联，复制使用即可）。', {
    type: 'object',
