...
const execute = async (node, context) => {
  ...
  try {
    ...
    const { selector, filePath, isDrop = false } = config
    // 获取所有需要处理的文件路径
    const filePaths = []
    ...
    // 处理上传按钮
    const handleUploadButton = async (targetFiles) => {
      try {
        // 等待目标元素出现
        ...
        const isMultiple = await page_eval(
          page,
          `(selector) => {
          const uploadButton = document.querySelector(selector)
    ...
    // 模拟文件拖放
    const handleDrop = async (targetFiles) => {
      try {
        // 等待目标元素出现
        await page.waitForSelector(selector)
        // 获取目标元素
    ...
    // 根据模式选择处理方法
    const handler = isDrop ? handleDrop : handleUploadButton
