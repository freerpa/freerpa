...
const automanBrowser = async (node, context) => {
  const { onNodeEvent, sendNodeEvent, next, onBeforeDestroy, apis, wait } = context
  const {
    envId,
    proxyUrl,
    other,
  ...
  try {
    ...
    if (offscreen) {
      ...
      const getViewBounds = () => {
        //更新其他窗口位置
        global.mainWindow.contentView.children.forEach((childView, index) => {
        ...
        //返回当前窗口位置
        return {
          x: global.mainWindow.contentView.children.length + 1,
          y: -envData.browser_height + 1,
          width: envData.browser_width,
          height: envData.browser_height
        }
      }
      ...
      window.on('show', () => {
        try {
          ...
          console.error('添加view到窗口失败:', error)
        }
      })
      ...
      window.on('hide', () => {
        try {
          ...
          console.error('添加view到主窗口失败:', error)
        }
      })
    ...
    if (envId) {
      ...
      // 等待页面加载完成
      try {
        await page.goto(envData.url, options)
      } catch (error) {
        console.log('访问页面失败:', error);
        throw error
      }
      await wait(500)
    }
