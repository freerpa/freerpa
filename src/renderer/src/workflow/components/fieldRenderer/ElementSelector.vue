...
<script setup>
  ...
  const displayName = computed(() => {
    ...
    try {
      const d = typeof value.value === 'string' ? JSON.parse(value.value) : value.value;
      return d.name || '';
    } catch {
      return '';
    }
    ...
    try {
      ...
      for (const es of sets) {
        ...
        if (parent) {
          ...
          uncatSets.push(setNode);
        }
      }

      if (uncatSets.length) {
        tree.push({ title: '未分类', key: 'cat:_uncat', children: uncatSets });
      }
  ...
  const onTreeSelect = (keys, { node }) => {
    ...
    editingElement.value = {
      ...
      selectors: (el.selectors || []).map((s, i) => ({
        _key: i + 2,
        type: s.type,
        text_subtype: s.text_subtype || 'equals',
