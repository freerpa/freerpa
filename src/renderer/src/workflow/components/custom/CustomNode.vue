<template>
  ...
  >
    ...
    <div v-if="data.deactivate" class="deactivate">
      <b>节点已停用（停用后不再执行）</b>
      <a-button size="small" type="primary" @click="data.deactivate = false" :disabled="isExecuting || isPreview">
...
<script setup>
  import { computed, defineAsyncComponent, ref, inject, provide, onUnmounted } from 'vue';
  ...
  const emit = defineEmits(['addNode', 'action', 'addNodeToSubFlow', 'showQuickConnect']);
  ...
  const { renameMode, nodeName, checkNodeName, saveNodeName, actionSelect } = useNodeActions(props, flowStore, emit);
