<template>
  <div class="node-header">
    <!-- Custom icon -->
    <a-avatar v-if="icon" :image-url="icon" :size="18" shape="square" class="node-icon" />

    <!-- Default definition icon with tooltip -->
    <a-tooltip v-else>
      <template #content>
        节点：{{ nodeDefinition.name }}
        <br />
        <div v-html="'描述：' + (nodeDefinition.description || '').replace(/\n/g, '<br />')"></div>
      </template>
      <component title="点击查看节点详情" :is="nodeDefinition.icon" class="node-icon" :style="iconStyle" />
    </a-tooltip>

    <!-- Node title -->
    <div class="node-title">
      <a-typography-text v-if="!renameMode" ellipsis style="margin: 0px" @dblclick="$emit('action', 'rename')">
        <span @click.stop>{{ name }}</span>
      </a-typography-text>
    ...
    <div class="node-actions">
      <a-space>
        <!-- Retrying badge -->
        <a-tag v-if="nodeStatus === 'retrying'" size="small" color="red">第 {{ errMsg }} 次重试中...</a-tag>
        ...
        <a-tooltip v-if="refreshable" content="刷新开发版插件">
          <icon-refresh class="node-refresh" :class="{ spin: refreshing }" @click.stop="$emit('refresh')" />
        </a-tooltip>
      </a-space>
    </div>
  </div>
...
<script setup>
  ...
  import { IconExclamationPolygonFill, IconBug, IconRefresh } from '@arco-design/web-vue/es/icon';
