...
export function useNodeDragDrop({
  ...
  const onNodeDrag = (data) => {
    ...
    const subFlowNodes = vueFlowRef.value.getNodes.filter((node) => node.type === 'subFlow' && !node.hidden);
    ...
    if (newNode) {
      if (sourceNode.position.x < targetNode.position.x) {
        const nextNodes = getAllSuccessorNodes(
          ...
          vueFlowRef.value.getNodes.filter(
            (node) => node.parentNode === sourceNode.parentNode && node.id !== newNode.id
          ),
          targetNode.id
        );
        // 更新节点位置为新节点让出位置
