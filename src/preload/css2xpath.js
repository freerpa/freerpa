var xpath_to_lower = function (s) {
  ...
  xpath_is_internal =
    ...
    xpath_url() +
    ',' +
    xpath_url_domain(xpath_ns_uri) +
    ') or ' +
    xpath_ends_with(xpath_url_domain(), xpath_url_domain(xpath_ns_uri)),
  xpath_is_local =
    '(' +
    xpath_has_protocal +
    ' and starts-with(' +
  ...
  css_combinators_regex =
    /\s*(!?[+>~,^ ])\s*(\.?\/+|[a-z\-]+::)?([a-z\-]+\()?((and\s*|or\s*|mod\s*)?[^+>~,\s'"\]\|\^\$\!\<\=\x1C-\x1F]+)?/g,
  css_combinators_callback = function (
    match,
    operator,
    axis,
    func,
  ...
  css_pseudo_classes_callback = function (match, name, g1, g2, arg, g3, g4, g5, offset, orig) {
    ...
    switch (
      ...
      case 'has-sibling':
        ...
        return (
          '[count(' + xpath + ') > 0 or count(following-sibling::' + xpath.substr(19) + ') > 0]'
        )
      case 'has-parent':
...
function prependAxis(s, axis) {
  return s.replace(regex_first_axis, function (match, start, literal) {
    if (literal.substr(literal.length - 2) === '::') {
      // Already has axis::
      return match
    }
