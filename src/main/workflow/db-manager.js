/**
 * @file: 外部数据库连接管理（主进程）
 * 统一经 knex 支持多种 SQL 数据库（MySQL/MariaDB、PostgreSQL、SQL Server、SQLite）。
...
/** 数据库类型 → knex client 名（knex 支持 mysql2/pg/mssql/sqlite3） */
const CLIENT_MAP = {
  mysql: 'mysql2',
  postgres: 'pg',
  mssql: 'mssql',
...
/** 归一化不同驱动返回结构（mysql2: [rows, fields]；pg: {rows}；mssql: {recordsets}；sqlite3: {rows}） */
const normalizeResult = (raw) => {
  ...
  if (raw && typeof raw === 'object') {
    if (Array.isArray(raw.recordsets) && raw.recordsets.length > 0) {
      return { rows: raw.recordsets[0], affectedRows: raw.rowsAffected?.[0] ?? 0 }
    }
