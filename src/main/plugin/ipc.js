...
import { getLatestVersionDir } from '../../renderer/src/workflow/utils/pluginLayout.js'
...
export const register = () => {
  ...
  ipcMain.handle('plugin:get', async (_, pluginId) => {
    ...
    // 附上 execute.js 源码（供渲染端配置面板展示）：findPlugin 已定位插件目录，直接取最高版本目录内的 execute.js
    if (found.info.hasExecute) {
      const latest = getLatestVersionDir(found.dir)
      const ep = latest && path.join(latest.dir, 'execute.js')
